...
//按钮布局
-(void)layoutBtnWithSize:(CGSize)size {

    for (NSInteger i=0; i<_numOfSubView ;i++) {// 178,245
        CGFloat yy=self.frame.size.height/2+sin((i/_numOfSubView)*M_PI*2+mStartAngle)*(self.frame.size.width/2-_subViewSize.width/2-(2/3)*self.frame.size.height-size.height/4);
        CGFloat xx=self.frame.size.width/2+cos((i/_numOfSubView)*M_PI*2+mStartAngle)*(self.frame.size.width/2-_subViewSize.width/2-(2/3)*self.frame.size.width-size.width/4);
...
//             NSLog(@"mTmpAngle is %f",mTmpAngle);
        ...
        [self layoutBtnWithSize:buttonSize];
